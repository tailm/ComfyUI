...
class PromptQueue:
    def __init__(self, server):
        ...
        self.db_path = os.path.join(folder_paths.get_user_directory(), "comfyui.db")
    ...
    def _init_history_db(self):
        """Initialize history database table if not exists."""
        import sqlite3
        try:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_history_prompt_id ON history(prompt_id)')
            conn.commit()
            conn.close()
        except Exception as e:
    ...
    def _load_history_from_db(self, user_id=None, max_items=None, offset=0, prompt_id=None):
        ...
        try:
            ...
            for row in cursor.fetchall():
                prompt_id, prompt, outputs, status, created_at = row
                results[prompt_id] = {
