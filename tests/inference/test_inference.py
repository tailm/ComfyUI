...
class ComfyGraph:
    ...
    def __init__(self, graph: dict, sampler_nodes: list[str]):
        self.graph = graph
        self.sampler_nodes = sampler_nodes

    def set_prompt(self, prompt, negative_prompt=None):
        # Sets the prompt for the sampler nodes (e.g. base and refiner)
        for node in self.sampler_nodes:
            prompt_input = self.graph[node]['inputs']['positive']
            prompt_node = prompt_input[0]
            self.graph[prompt_node]['inputs']['text'] = prompt
            if negative_prompt:
                negative_prompt_node = (
                    self.graph[node]['inputs']['negative'][0]
                )
                graph_inputs = self.graph[negative_prompt_node]['inputs']
                graph_inputs['text'] = negative_prompt

    def set_sampler_name(self, sampler_name: str):
        # Sets the sampler name for the sampler nodes (e.g. base and refiner)
        for node in self.sampler_nodes:
            self.graph[node]['inputs']['sampler_name'] = sampler_name

    def set_scheduler(self, scheduler: str):
        # Sets the scheduler for the sampler nodes (e.g. base and refiner)
        for node in self.sampler_nodes:
...
def get_required_checkpoint_names(graph: dict) -> set[str]:
    return {
        node_data['inputs']['ckpt_name']
        for node_data in graph.values()
        if node_data.get('class_type') == 'CheckpointLoaderSimple'
        and isinstance(
            node_data.get('inputs', {}).get('ckpt_name'),
            str,
        )
    }


def ensure_checkpoints_available(graph: dict):
    checkpoint_dir = (
        Path(__file__).resolve().parents[2]
        / 'models'
        / 'checkpoints'
    )
    required = get_required_checkpoint_names(graph)
    if not checkpoint_dir.exists():
        return False, checkpoint_dir, required
    ...
    return len(missing) == 0, checkpoint_dir, missing
...
class ComfyClient:
    ...
    def queue_prompt(self, prompt):
        ...
        req = urllib.request.Request(
            'http://{}/prompt'.format(self.server_address),
            data=data,
        )
...
DEFAULT_COMFY_GRAPH = ComfyGraph(
    graph=default_graph,
    sampler_nodes=['10', '14'],
)

DEFAULT_COMFY_GRAPH_ID = os.path.splitext(
    os.path.basename(default_graph_file)
)[0]
...
class TestInference:
    ...
    def _server(self, args_pytest):
        available, checkpoint_dir, missing = (
            ensure_checkpoints_available(default_graph)
        )
        if not available:
            if checkpoint_dir.exists():
                pytest.skip(
                    'Skipping inference tests because required '
                    'checkpoint file(s) are missing from {0}: {1}'.format(
                        checkpoint_dir,
                        missing,
                    )
                )
            pytest.skip(
                'Skipping inference tests because checkpoint directory '
                '{0} is missing.'.format(
                    checkpoint_dir,
                )
            )

        # Start server with an in-memory database to avoid file-backed Alembic
        # migration issues.
    ...
    #
    # Returns a "_client_graph", which is a client-graph pair corresponding to
    # an initialized server
    # The "graph" is the default graph
    @fixture(
        scope='class',
        params=comfy_graph_list,
        ids=comfy_graph_ids,
        autouse=True,
    )
    def _client_graph(
        ...
        args_pytest,
        _server,
    ) -> tuple[ComfyClient, ComfyGraph]:
        comfy_graph = request.param

        # Start client
        comfy_client = self.start_client(
            args_pytest["listen"],
            args_pytest["port"],
        )
