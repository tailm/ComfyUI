...
3. Configure environment (optional):
   Create a `.env` file in the project root based on the provided [.env_example](config/.env_example) file.
...
Any `*.comfy.org` URL automatically enables cloud mode, which includes the GCS media proxy needed for viewing generated images and videos. See [.env_example](config/.env_example) for all available cloud URLs.
